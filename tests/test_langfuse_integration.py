...
def request_context(basic_request_params):
    ...
    ctx.client_metadata = {"user_id": "test-user", "session_id": "test-session"}
    return ctx
...
class TestLangfuseMiddlewareSetup:
    ...
    def test_langfuse_middleware_custom_config(
    ...
    def test_langfuse_client_caching(self, mock_langfuse_module, request_context):
        ...
        middleware_setup = langfuse_middleware(
            public_key="pk-cache-test", secret_key="sk-cache-test"
        )
    ...
    def test_langfuse_not_available(self, request_context):
        ...
        pytest.skip(
            "Cannot easily simulate missing langfuse package in test environment"
        )
...
class TestPromptManagement:
    ...
    def test_prompt_management_with_prompt_id(
        self, mock_langfuse_module, request_context
    ):
        ...
        mock_langfuse_module[
            "prompt_client"
        ].compile.return_value = "Hello Alice, let's talk about AI"

        # Call prompt management
        _prompt_management_pre_llm(request_context)

        # Verify prompt was fetched and compiled
        mock_langfuse_module["client"].get_prompt.assert_called_once_with(
            "test-prompt", label="latest"
        )
        mock_langfuse_module["prompt_client"].compile.assert_called_once_with(
            name="Alice", topic="AI"
        )
        ...
        assert (
            request_context.request.messages[0]["content"]
            == "Hello Alice, let's talk about AI"
        )

    def test_prompt_management_with_messages_array(
        self, mock_langfuse_module, request_context
    ):
        ...
        request_context.request.__pydantic_extra__ = {"prompt_id": "test-prompt-array"}
    ...
    def test_prompt_not_found_error(self, mock_langfuse_module, request_context):
        ...
        request_context.request.__pydantic_extra__ = {"prompt_id": "nonexistent-prompt"}
        ...
        mock_langfuse_module["client"].get_prompt.side_effect = NotFoundError(
            "Prompt not found"
        )

        # Should raise MiddlewareError
        with pytest.raises(
            MiddlewareError, match="Langfuse prompt 'nonexistent-prompt' not found"
        ):
            _prompt_management_pre_llm(request_context)

    def test_prompt_management_custom_label(
        self, mock_langfuse_module, request_context
    ):
        ...
        request_context.request.__pydantic_extra__ = {"prompt_id": "test-prompt"}
...
class TestObservability:
    ...
    def test_generation_tracking(
        self, mock_langfuse_module, request_context, mock_completion_response
    ):
        """Test generation tracking through pre and post LLM hooks."""
        from sllmp.middleware.service.langfuse import langfuse_middleware
        from sllmp.middleware.service.langfuse.util import (
            extract_chat_prompt,
            process_output,
        )
        ...
        assert (
            len(request_context.pipeline.llm_call) == 2
        )  # pre and post callbacks added
    ...
    def test_response_complete_tracking(
        self, mock_langfuse_module, request_context, mock_completion_response
    ):
...
class TestIntegrationWithPipeline:
    ...
    def test_full_pipeline_integration(
        self, mock_langfuse_module, basic_request_params, mock_completion_response
    ):
        ...
        middleware_setup = langfuse_middleware(
            public_key="pk-integration", secret_key="sk-integration"
        )
    ...
    def test_middleware_error_handling(self, mock_langfuse_module, request_context):
        ...
        middleware_setup = langfuse_middleware(
            public_key="pk-error-test", secret_key="sk-error-test"
        )
